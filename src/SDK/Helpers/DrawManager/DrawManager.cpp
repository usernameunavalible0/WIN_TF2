...
void CHelpers_DrawManager::Initialize()
{
	...
	auto GenTexture = [&](const char* szFilename)
	{
		const int nTextureIdOut = I::MatSystemSurface->CreateNewTextureID(false);
		I::MatSystemSurface->DrawSetTextureFile(nTextureIdOut, szFilename, 0, true);
		return nTextureIdOut;
	};

	I::MatSystemSurface->DrawSetTextureFile(m_Textures[ETextures::ICON_DEMO] = I::MatSystemSurface->CreateNewTextureID(false), "hud/leaderboard_class_demo", 0, true);
	m_Textures[ETextures::ICON_ENGI] = GenTexture("hud/leaderboard_class_engineer");
	m_Textures[ETextures::ICON_HEAVY] = GenTexture("hud/leaderboard_class_heavy");
	m_Textures[ETextures::ICON_MEDIC] = GenTexture("hud/leaderboard_class_medic");
	m_Textures[ETextures::ICON_PYRO] = GenTexture("hud/leaderboard_class_pyro");
	m_Textures[ETextures::ICON_SCOUT] = GenTexture("hud/leaderboard_class_scout");
	m_Textures[ETextures::ICON_SNIPER] = GenTexture("hud/leaderboard_class_sniper");
	m_Textures[ETextures::ICON_SOLDIER] = GenTexture("hud/leaderboard_class_soldier");
	m_Textures[ETextures::ICON_SPY] = GenTexture("hud/leaderboard_class_spy");
}
...
void CHelpers_DrawManager::UnInitialize()
{
	...
	for (std::pair<const ETextures, int>& tex : m_Textures)
		I::MatSystemSurface->DestroyTextureID(tex.second);
...
void CHelpers_DrawManager::UpdateMatrix()
{
	...
	if (I::BaseClientDLL->GetPlayerView(ViewSetup))
	{
		...
		//TODO: IVEngineClient::WorldToScreenMatrix exists???
...
void CHelpers_DrawManager::Texture(const ETextures& tex, int x, int y, const int w, const int h, const short align)
{
	const int id = m_Textures[tex];
	...
	if (align)
	{
		...
		if (align & TEX_CENTERX)
			x -= (w / 2);

		if (align & TEX_CENTERY)
			y -= (h / 2);
	}
