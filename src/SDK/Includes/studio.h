...
struct mstudiovertanim_wrinkle_t : public mstudiovertanim_t
{
	...
	{
		int nWrinkleDeltaInt = static_cast<int>(flWrinkle / flVertAnimFixedPointScale);
		wrinkledelta = std::clamp(nWrinkleDeltaInt, -32767, 32767);
	}

	inline Vector4D GetDeltaFixed(float flVertAnimFixedPointScale)
	{
		return { delta[0] * flVertAnimFixedPointScale, delta[1] * flVertAnimFixedPointScale, delta[2] * flVertAnimFixedPointScale, wrinkledelta * flVertAnimFixedPointScale };
	}
...
{
	//TODO: Is ownerID 8 bytes on win64?
	volatile unsigned __int32 m_ownerID;
	int						  m_depth;
};
...
//TODO: Did this change in the 64 bit upgrade?
...
public:
	...
	private:
		...
		void SetValidationPair(const CStudioHdr* __restrict pstudiohdr);
...
inline void Studio_SetRootLOD(studiohdr_t* pStudioHdr, int rootLOD)
{
	...
	rootLOD = std::clamp(rootLOD, 0, MAX_NUM_LODS - 1);
